import { addDays, daysBetween, toDayKey } from './math';
import { computeErrorPenalty, resolveDominantErrorType } from './errorInference';
import {
...
import { projectExamRetentionProbability } from './readiness';
import {
...
import { computeMasteryLevel, updateStabilityScore } from './stability';
import {
  AttemptInput,
...
const updateMedianApproximation = (previousMedian: number | null, nextValue: number | null) => {
...
export const updateQuestionState = ({
  ...
  const incorrectAttempts = (previousState?.incorrectAttempts ?? 0) + (attempt.isCorrect ? 0 : 1);
  ...
  const distinctSuccessfulDays =
    attempt.isCorrect &&
    (!previousState?.lastCorrectAt || toDayKey(previousState.lastCorrectAt) !== toDayKey(now))
  ...
  return {
    ...
    nextState: {
      ...
      lastCorrectAt: attempt.isCorrect ? now.toISOString() : (previousState?.lastCorrectAt ?? null),
      nextReviewAt,
      masteryLevel,
      stabilityScore,
      ...
      fastCorrectCount:
        (previousState?.fastCorrectCount ?? 0) + (attempt.isCorrect && latencyFactor >= 1 ? 1 : 0),
      slowCorrectCount:
        (previousState?.slowCorrectCount ?? 0) + (attempt.isCorrect && latencyFactor < 1 ? 1 : 0),
      lapseCount,
