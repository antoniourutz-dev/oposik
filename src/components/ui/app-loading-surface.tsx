...
type AppLoadingSurfaceProps = {
  /** Si se omite, solo hay spinner (sin mensajes internos visibles). */
  label?: string;
...
export function AppLoadingSurface({ label, ariaLabel }: AppLoadingSurfaceProps) {
  ...
  return (
    <div
      ...
      aria-live="polite"
      aria-busy="true"
      aria-label={ariaLabel ?? visibleLabel ?? 'Cargando'}
    >
...
/**
 * Reservado para estados de cuenta/catálogo: no mostramos mensajes internos al alumno.
 */
export function CatalogSyncBanner(_props: {
...
/** Panel de carga dentro del shell — misma familia visual que AppLoadingSurface. */
  ...
  return (
    ...
    >
      <div className="relative w-full overflow-hidden p-8 text-center ui-surface-solid">
        ...
        {visible ? (
          <p className="relative mt-5 text-sm font-black uppercase tracking-[0.18em] text-slate-600">{visible}</p>
        ) : null}
      </div>
    </div>
  );
