...
const ChartsFallback = () => (
  <div
    className="space-y-3 rounded-2xl border border-slate-100/90 bg-white/70 p-5"
    role="status"
    aria-live="polite"
    aria-busy="true"
  >
...
const DashboardStatsTab: React.FC<DashboardContentProps> = ({
  ...
  const newCountResolved =
    learningDashboard?.newCount ?? Math.max(totalQuestionsResolved - seenQuestionsResolved, 0);
  ...
  const maxRiskShare = topRiskBreakdown.reduce((max, risk) => {
    const share =
      riskBreakdownTotal === 0 ? 0 : Math.round((risk.count / riskBreakdownTotal) * 100);
  ...
  const chronodata = recentSessions.reduce(
    (acc: Record<string, { correct: number; total: number }>, s) => {
  ...
  const bestSlot = Object.entries(chronodata)
    .map(([slot, data]) => ({
      slot,
      accuracy:
        (data as { total: number; correct: number }).total > 0
          ? (data as { total: number; correct: number }).correct /
            (data as { total: number; correct: number }).total
          : 0,
    }))
  ...
  const masteryDonutData = [
    ...
    { name: 'Frágiles', value: fragileCountResolved, color: '#f59e0b' },
    {
      name: 'Vistas',
      value:
        seenQuestionsResolved - (solidCountResolved + masteredCountResolved + fragileCountResolved),
      color: '#7cb6e8',
    },
  ].filter((d) => d.value > 0);
  ...
  const performanceBarData = topWeakCategories.slice(0, 5).map((cat) => ({
    ...
    accuracy: Math.round(
      ((cat.attempts - cat.incorrectAttempts) / Math.max(1, cat.attempts)) * 100,
    ),
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="flex-1">
            ...
            <p className="mt-3 max-w-2xl text-[15px] font-semibold leading-relaxed text-slate-600">
              Basado en tu curva de olvido y la densidad del temario, tu sesión óptima de hoy debe
              centrarse en el{' '}
              <span className="text-slate-900 font-bold">Bloque de Sanidad (Tema 4)</span>. Corregir
              este "gap" elevará tu Readiness global a un{' '}
              <span className="text-emerald-600 font-bold">78%</span> antes del simulacro de mañana.
            </p>
          </div>
      ...
      <div className="mx-auto w-full max-w-[1780px] space-y-12">
          ...
          <div className="space-y-8 lg:col-span-8">
            ...
            <section className="relative overflow-hidden rounded-[2.5rem] bg-slate-950 p-8 shadow-2xl xl:p-10">
              <div className="relative z-10">
                ...
                <h2 className="mt-8 text-2xl font-black text-white lg:text-3xl">
                  {statsLeadLabel}
                </h2>
                ...
                <div className="mt-10 flex flex-wrap items-center gap-6 border-t border-white/10 pt-8">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-sm font-black text-white">
                      {goldenWindow}{' '}
                      {typeof goldenAccuracy === 'number' && goldenAccuracy > 0
                        ? `(${goldenAccuracy}%)`
                        : ''}
                    </p>
                  </div>
              ...
              <div className="mb-10 flex items-center justify-between">
                <div>
                  ...
                  <p className="mt-1 text-xs font-bold text-slate-400 uppercase tracking-widest">
                    Evolución de los últimos cierres
                  </p>
                </div>
          ...
          <div className="space-y-8 lg:col-span-4">
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-1">
              ...
              <div className="hidden lg:block">
                ...
                <SectionCard
                  title="Huella Cognitiva"
                  hint="Análisis 360º de tu comportamiento"
                  className="mt-8"
                >
                  <div className="mt-6 flex flex-col items-center gap-6 lg:flex-row">
                    <div className="flex-1 w-full min-h-[300px]">
                      {charts ? (
                        <charts.BehavioralRadarChart
                          data={[
                            {
                              subject: 'Acierto',
                              value: toPercentNumber(resolvedExamReadinessRate),
                            },
          ...
          <section className="hidden lg:block rounded-[2.5rem] border border-slate-100 bg-white p-8 shadow-sm">
            ...
            <div className="flex flex-col items-center gap-8">
              <div className="w-full max-w-[240px]">
                {charts ? (
                  <charts.DistributionDonutChart data={masteryDonutData} />
                ) : (
                  <ChartsFallback />
                )}
              </div>
                  ...
                  >
                    <div className="flex items-center gap-2 text-[11px] font-black text-slate-500 uppercase tracking-tight">
                      <span
                        ...
                        style={{ backgroundColor: d.color } as React.CSSProperties}
                      />
                      {d.name}
                    </div>
          ...
          <section className="rounded-[2.5rem] border border-slate-100 bg-white p-8 shadow-sm lg:col-span-2">
            <div className="mb-8 flex items-end justify-between">
              <div>
                ...
                <p className="mt-1 text-xs font-bold text-slate-400 uppercase tracking-widest">
                  Temas con mayor desviación
                </p>
              </div>
              <button className="hidden lg:block text-xs font-black uppercase text-quantia-pink">
                Ver desglose completo
              </button>
            </div>
            <div className="h-[280px] w-full">
              {charts ? (
                <charts.PerformanceBarChart data={performanceBarData} />
              ) : (
                <ChartsFallback />
              )}
            </div>
        ...
        <div className="grid gap-8 lg:grid-cols-12">
            ...
            <section className="lg:col-span-12 rounded-[2.5rem] border border-slate-100 bg-white p-8 shadow-sm">
              <div className="mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                  ...
                  <p className="mt-1 text-xs font-bold text-slate-400 uppercase tracking-widest">
                    Dominio por marco legal (Resumen real)
                  </p>
                </div>
              ...
              <div className="mt-8 flex items-center gap-3 rounded-2xl bg-slate-50 p-4 border border-slate-100">
                ...
                <p className="text-[11px] font-semibold text-slate-600">
                  <span className="font-black text-slate-900 uppercase">Coach Insight:</span>{' '}
                  Análisis de persistencia normativa activo. Tus puntos débiles legales aparecerán
          ...
          {(!learningDashboardV2?.lawBreakdown ||
            learningDashboardV2.lawBreakdown.length === 0) && (
            <section className="lg:col-span-12 rounded-[2.5rem] border border-dashed border-slate-200 bg-slate-50/50 p-12 text-center">
              ...
              <h3 className="mt-4 text-lg font-black text-slate-900">
                Radar Normativo en Calibración
              </h3>
              <p className="mx-auto mt-2 max-w-sm text-sm font-semibold text-slate-500">
                Estamos esperando los primeros datos de tu nueva columna{' '}
                <code className="text-indigo-600">ley_referencia</code>. Realiza unos entrenamientos
                para que el motor de IA empiece a agrupar tus aciertos.
              </p>
            </section>
          )}
          ...
          <section className="overflow-hidden rounded-[2.5rem] border border-slate-100 bg-white shadow-sm lg:col-span-8">
            <div className="p-8">
              ...
              <p className="mt-1 text-xs font-bold text-slate-400 uppercase tracking-widest italic">
                Riesgo detectado por tema
              </p>
            </div>
                  ...
                  {topWeakCategories.slice(0, 6).map((cat) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-6 py-6 hidden sm:table-cell text-xs font-bold text-slate-500">
                          {cat.attempts} preg.
                        </td>
                        ...
                        <td className="px-8 py-6 text-right">
                          <span
                            className={`inline-flex px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-tighter ${acc < 50 ? 'bg-rose-50 text-rose-600' : acc < 75 ? 'bg-amber-50 text-amber-600' : 'bg-emerald-50 text-emerald-600'}`}
                          >
        ...
        <footer className="rounded-[2rem] bg-slate-100/50 p-6 lg:p-10 flex flex-col lg:flex-row items-center gap-6 text-center lg:text-left">
          ...
          <div>
            ...
            <p className="mt-1 text-sm font-semibold text-slate-600 leading-relaxed italic">
              ...
              Se recomienda un mínimo de 3 simulacros semanales para maximizar la fidelidad del
              Ready Index."
            </p>
          </div>
        </footer>
