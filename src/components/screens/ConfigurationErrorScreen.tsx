...
const ConfigurationErrorScreen: React.FC<ConfigurationErrorScreenProps> = React.memo(
  ({ missingVars }) => {
    return (
        ...
        <section className="w-full max-w-2xl overflow-hidden rounded-[2rem] border border-amber-200 bg-white shadow-[0_30px_80px_-35px_rgba(120,53,15,0.35)]">
          ...
          <div className="space-y-5 px-6 py-6 text-slate-700">
            <p className="text-sm font-medium leading-relaxed">
              La aplicacion no puede arrancar correctamente porque el build no recibio la
            ...
            <div className="rounded-[1.5rem] border border-sky-100 bg-sky-50 px-5 py-4">
              <p className="text-sm font-bold text-sky-900">
                En Vercel tienes que definirlas en Project Settings &gt; Environment Variables y
                volver a desplegar.
              </p>
              <p className="mt-2 text-sm font-medium leading-relaxed text-sky-800">
                Si quieres probar en local, crea tambien un archivo <code>.env.local</code> con esos
                mismos valores.
              </p>
            </div>
          </div>
