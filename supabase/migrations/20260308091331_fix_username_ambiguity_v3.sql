-- Remove the old function with the ambiguous OUT parameters
drop function if exists app.change_my_username(text, text, uuid, jsonb);

-- Recreate with non-colliding OUT parameter names
...
begin
  ...
  return query
  select v_actor_user_id as out_user_id, v_old_username as out_old_username, v_new_username as out_new_username, v_now as out_changed_at;
