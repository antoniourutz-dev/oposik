...
update app.practice_profiles
set curriculum = coalesce(nullif(trim(curriculum), ''), 'general')
where curriculum is null
   or trim(curriculum) = ''
   or curriculum <> coalesce(nullif(trim(curriculum), ''), 'general');

alter table app.practice_profiles
  drop constraint if exists practice_profiles_pkey;

alter table app.practice_profiles
  add constraint practice_profiles_pkey primary key (user_id, curriculum);

create or replace function app.ensure_practice_profile(
  p_user_id uuid,
...
begin
  insert into app.practice_profiles (user_id, curriculum)
...
create or replace function app.get_my_practice_profile_for_curriculum(
  p_curriculum text default 'general'
)
returns table (
  user_id uuid,
...
begin
  return query
  select *
  from app.get_my_practice_profile_for_curriculum('general');
end;
$$;

create or replace function app.admin_get_practice_profile_for_curriculum(
  p_user_id uuid,
  p_curriculum text default 'general'
)
returns table (
...
begin
  ...
  perform app.ensure_practice_profile(p_user_id, v_curriculum);
...
begin
  return query
  select *
  from app.admin_get_practice_profile_for_curriculum(p_user_id, 'general');
end;
$$;

create or replace function app.admin_get_recent_practice_sessions_for_curriculum(
  p_user_id uuid,
...
begin
  return query
  select *
  from app.admin_get_recent_practice_sessions_for_curriculum(p_user_id, p_limit, 'general');
end;
$$;
...
grant execute on function app.ensure_practice_profile(uuid, text) to authenticated;
grant execute on function app.get_my_practice_profile_for_curriculum(text) to authenticated;
grant execute on function app.admin_get_practice_profile_for_curriculum(uuid, text) to authenticated;
grant execute on function app.admin_get_recent_practice_sessions_for_curriculum(uuid, integer, text) to authenticated;
